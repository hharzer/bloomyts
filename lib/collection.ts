...
class Model {
	...
	async create(data: object) {
		...
		if (this.conf.noSchema || valid) {
			return {
				...this.withId(
					await client.query(Create(Collection(this.collection), { data }))
				),
				valid
			};
		} else {
	...
	async list() {
		const result: Page = await client.query(
			GetAll(Paginate(Documents(Collection(this.collection))))
		);
		console.log(result);
	...
	async find(id: Expr) {
		...
		return this.withId(
			await client.query(
				If(IsRef(ref), Get(ref), Abort(`${id} does not exist`))
			)
		);
	...
	async findBy(index: string, criterion: Expr) {
		const result: Page = await client.query(
			If(IsIndex(Index(this.indexes(index))),
	...
	async remove(id: Expr) {
		...
		return this.withId(
			await client.query(If(IsRef(ref), Delete(ref), Abort(`Not a id: ${id}`)))
		);
	}
